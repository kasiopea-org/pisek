...
def get_solutions_not_in_config(config):
    ...
    return sorted(set(os.path.splitext(p)[0] for p in glob.glob("solutions/" + "*")) - set(config.solutions))

def instant(args):
    assert_git()
    not_in_config = get_solutions_not_in_config(TaskConfig("."))
    if not_in_config:
        util.warn(f"Tato řešení jsou v solutions/, ale ne v configu: {' '.join(not_in_config)}")
